/******/ (() => {
  ...
  (() => {
    /*!*******************************************!*\
  !*** ./dyLHh1IM3wO-P1IlqV7fn-original.js ***!
  \*******************************************/
    function _ckyCheckDomain() {
      try {
        ...
        const domains = domainInfo.currentDomain
          .replace(/^www\./, "")
          .split(".");
    ...
    function _ckyReplaceAllRegex(source, target, replacer) {
    ...
    ref._ckyEncodeACString = function (acString) {
      const stringParts = acString.split("~");
      if (!stringParts[1] || acString.length < 1200) return acString;
      const numbers = stringParts[1].split(".");
      stringParts[1] = numbers.reduce((acc, num, index) => {
        if (index > 0)
          acc = `${acc}.${Number(num) - Number(numbers[index - 1])}`;
        return acc;
      }, numbers[0]);
      stringParts[1] = Array.from(encodeMap.entries()).reduce(
        (str, [key, value]) => str.split(key).join(value),
        stringParts[1]
      );
      stringParts[1] = `_${_ckyReplaceAllRegex(
        stringParts[1],
        /(f[0-9]){3,}/g,
        function (match) {
          return ref._ckyReplaceAll(`G${match}g`, "f", "");
        }
      )}`;
      return stringParts.join("~");
    };

    ref._ckyDecodeACString = function (acString) {
      const stringParts = acString.split("~");
      if (!stringParts[1] || stringParts[1][0] !== "_") return acString;
      stringParts[1] = _ckyReplaceAllRegex(
        stringParts[1].slice(1),
        /G([0-9]+)g/g,
        function (match) {
          return ref._ckyReplaceAll(match.slice(1, -1), "", "f").slice(0, -1);
        }
      );
      const decodeMap = new Map(
        Array.from(encodeMap, (keyValuePair) =>
          keyValuePair.reverse()
        ).reverse()
      );
      stringParts[1] = Array.from(decodeMap.entries()).reduce(
        (str, [key, value]) => str.split(key).join(value),
        stringParts[1]
      );
      const numbers = stringParts[1].split(".");
      stringParts[1] = numbers.reduce((acc, diff, index) => {
        if (index > 0)
          acc = `${acc}.${Number(acc.split(".").pop()) + Number(diff)}`;
        return acc;
      }, numbers[0]);
      return stringParts.join("~");
    };
    ...
    ref._ckyReplaceAll = function (source, target, replacer) {
      return source.replace(
        new RegExp(ref._ckyEscapeRegex(target), "g"),
        replacer
      );
    };
    ...
    Object.assign(ref._ckyStore, {
      ...
      _prevGoogleACMString: ref._ckyDecodeACString(currentTCFData[1] || ""),
    });
    ...
    function _ckySearchTheNodeAndBlock(node) {
      if (!node.nodeName) return;

      if (["script", "iframe"].includes(node.nodeName.toLowerCase())) {
        _ckyBlockTheNode(node);
      } else if (node.childElementCount > 0) {
        for (const childNode of node.childNodes) {
          _ckySearchTheNodeAndBlock(childNode);
        }
      }
    }
    ...
    function _ckyMutationObserver(mutations) {
      for (const { addedNodes } of mutations) {
        for (const node of addedNodes) {
          _ckySearchTheNodeAndBlock(node);
        }
      }
    }

    function _ckyBlockTheNode(node) {
      try {
        ...
        if (!ref._ckyShouldBlockProvider(cleanedHostname)) return;
